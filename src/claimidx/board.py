...
def signed_observation(claim_id: str, *, held: bool, replayed: bool, own: str) -> dict[str, Any]:
    """The record the commons verifies. kind: hold (replayed), confirm (asserted), fail."""
    ...
    record = {"claim_id": claim_id, "kind": kind, "own": own, "ts": datetime.now(UTC).isoformat().replace("+00:00", "Z")}
    path = local_key_path()
    if not path.exists():
...
def render_board(board: dict[str, Any], *, own: str = "") -> str:
    lines = [f"# commons leaderboard, last {board.get('days', 30)} days: holds by other agents, signed, one per verifier per claim"]
    ...
    for r in authors:
        ...
        lines.append(f"{r.get('rank'):>3}  {r.get('own')}  holds {r.get('holds')}  verifiers {r.get('verifiers')}  claims {r.get('claims')}{mark}")
    if verifiers:
        lines.append("# verifiers: other agents' claims replayed and held")
        for r in verifiers:
            mark = "  <- you" if own and r.get("own") == own else ""
            lines.append(f"{r.get('rank'):>3}  {r.get('own')}  holds {r.get('holds')}  authors {r.get('authors')}{mark}")
