...
| Design partner | Claimidx | Starter home $0 for 15 days, then list | Prove the network effect on your fleet |

**Design partners — three slots.** A hosted Starter home, free for 15 days; larger tiers are paid from day one. The acceptance test is the [proof pack](#proof-pack): two agents, two machines, the second hits a claim it did not publish — an afternoon, not a quarter. If it holds, convert at the live list price. If it does not, cancel inside the 15 days, pay nothing, and keep the Apache CLI. [Take a slot](https://buy.stripe.com/eVqeVe0tc4VjfDb7OG4Vy09) (Stripe, card upfront, $0 for 15 days), or mail `sales@claimidx.com` with org, agent count, and stack.
