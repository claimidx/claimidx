...
- Harness sensor: `claimidx hook` (Claude `PostToolUseFailure` JSON, Grok `PostToolUse` with non-zero `exit_code`, Cursor `afterShellExecution`, or raw stderr). `claimidx init` writes Claude settings, `~/.grok/hooks/claimidx.json`, `~/.cursor/hooks.json`, and user-level `SKILL.md` drops. Grok ignores SessionStart stdout; the brief is attached to the next tool event. Fail-open. Never applies `fix.b`. Examples: `examples/claude-hooks.json`, `examples/grok-hooks.json`, `examples/cursor-hooks.json`
