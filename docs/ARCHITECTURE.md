...
- Ask surfaces `evidence` (`retrieved` vs `reproduced` when this consumer's `nr` held), `match` (`exact` fp vs `similar`), `lights` (`prior_art` / `integrity` / `recovery` — keep independent), overlapping error `tokens`, `untrusted`, `age_days`, `dep_drift`, `eval_proof` (recipe-per-fp, not query-err match; 1.08 does not break sibling ties; the recipe-per-fp warn fires when the query err string differs from the stored canonical row), `nr`, `src`, `nf`, `warn` (`normalization_risk`, `nc without replay`, `rt omitted`). A hit is evidence, not a command: retrieve → reason → attempt → observe → verify → update. Hook miss prints `CLAIMIDX miss`; empty extract stays silent unless optional absence telemetry (`CLAIMIDX_HOOK_ABSENCE` / `--absence`) emits `delivered: false`.
...
- Harness sensor: `claimidx hook` (Claude `PostToolUseFailure` JSON, Grok/Codex `PostToolUse` with non-zero `exit_code`, Cursor `afterShellExecution`, Gemini `AfterTool`, or raw stderr). `claimidx init` writes Claude settings, `~/.grok/hooks/claimidx.json`, `~/.cursor/hooks.json`, `~/.codex/hooks.json`, Gemini `AfterTool`, user-level `SKILL.md` drops, and MCP into Cursor/Grok/Codex/Gemini/OpenCode/VS Code/Cline/Continue/Windsurf when those configs exist. Grok ignores SessionStart stdout; the brief is attached to the next tool event. Fail-open. Never applies `fix.b`. Examples: `examples/claude-hooks.json`, `examples/grok-hooks.json`, `examples/cursor-hooks.json`, `examples/codex-hooks.json`, `examples/gemini-hooks.json`
- MCP: full catalog by default; `CLAIMIDX_MCP_TOOLS=core` lists ask/claim/apply/run/hook/whoami/doctor. `claimidx_run` is the hook for harnesses without one.
- CI: `.github/actions/run` wraps `claimidx run --`
...
- Local DID lifecycle funnel (install->init->ask->sync->confirm->publish->share) is derived from the event log for Growth drop-off counts; `claimidx funnel` (no serve) / `impact` / `doctor` / `GET /api/funnel`. Stage events stay on the local home SQLite — not projected to commons. `claimidx funnel --commons` adds a public-ledger owner proxy (countable/held/confirmed), not install->init drop-off. Cursor-based protocol events are idempotent and batch-hashed. `share-preview` exposes the privacy projection before transport.
