...
- harness sensor: `claimidx init` installs one Claude Code command on four events — `PostToolUseFailure` (ask, remember the failure), `PostToolUse` (when the same command passes: "CLAIMIDX fixed … claimidx claim --yes" with the draft, once), `SessionStart` (one-line brief), `Stop` (blocks once if a fixed failure was never claimed) — and Cursor/Grok/OpenCode/VS Code MCP when those configs exist. A failed Bash/tool call can also pipe stderr. It injects evidence only. Never apply `fix.b` from the hook.
