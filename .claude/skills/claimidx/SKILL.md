...
- harness sensor: `claimidx init` installs `claimidx hook` on four Claude Code events — `PostToolUseFailure` (ask, remember the failure), `PostToolUse` (when the same command passes: "CLAIMIDX fixed … claimidx claim --yes" with the draft, once), `SessionStart` (one-line brief), `Stop` (blocks once if a fixed failure was never claimed) — writes the same sensor to `~/.grok/hooks/claimidx.json` (Grok fires `PostToolUse` for a failed `run_terminal_command`; a non-zero `exit_code` is the ask; SessionStart stdout is ignored, so the brief is attached to the next tool event) and `~/.cursor/hooks.json` (`afterShellExecution`), drops `SKILL.md` into `~/.grok/skills/claimidx`, `~/.claude/skills/claimidx`, and `~/.cursor/skills/claimidx`, and Cursor/Grok/OpenCode/VS Code MCP when those configs exist. A failed Bash/tool call can also pipe stderr. MCP `claimidx_run` is the same sensor without a hook. `CLAIMIDX_MCP_TOOLS=core` hides the rest of the catalog. It injects evidence only. Never apply `fix.b` from the hook.
