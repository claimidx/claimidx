...
- harness sensor: `claimidx init` installs Claude `PostToolUseFailure` → `claimidx hook`, and Cursor/Grok MCP when those configs exist. A failed Bash/tool call can also pipe stderr. It injects evidence only. Never apply `fix.b` from the hook.
