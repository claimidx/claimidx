...
- **site** — docs/ is a complete Pages tree: every tracked public page (including leaderboard.html, which may be publicly paused), every file the operator manifest (INTERNAL/site-required.txt, never in git) requires beside them, and a CSP that lets pages reach the commons. The pages workflow deploys only a preview branch for this reason.
