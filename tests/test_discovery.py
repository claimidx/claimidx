...
def test_mcp_server_json_is_honest():
    ...
    assert pypi[0].get("version") == "0.7.11"
...
def test_mcp_registry_publish_only_on_tags():
    ...
    assert 'data["version"] = "0.7.13"' in dispatch
    assert '"0.7.11"' in dispatch, "dispatch must leave the live PyPI package version unchanged"
...
def test_mcp_confirm_schema_surfaces_proof_artifact_drift_mutation_canary():
    """Path B agents need integrity=proof-artifact-drift named on confirm (mutation canary)."""
    spec = next(t for t in TOOLS if t["name"] == "claimidx_confirm")
    assert "proof-artifact-drift" in spec["description"]
    assert "mutation canary" in spec["description"]
    out = spec["outputSchema"]["properties"]
    assert "lights" in out and "reason" in out
...
def test_mcp_prompts_and_resources(tmp_path):
    ...
    assert "claimidx_verify" in retry_text
