...
def test_mcp_server_json_is_honest():
    ...
    assert pypi[0].get("version") == "0.7.9"
...
def test_mcp_registry_publish_only_on_tags():
    ...
    assert 'data["version"] = "0.7.11"' in dispatch
    assert '"0.7.9"' in dispatch, "dispatch must leave the live PyPI package version unchanged"
