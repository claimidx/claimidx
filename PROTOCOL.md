...
| `verify` | batch replay. Confirm if the eval held. Fail only on a proven miss. Skip builtin `true`/`false`, missing trees, missing interpreters, and evals that cannot prove the pin. `verify --dry-run` lists chosen claims and does not run evals, venv, or pip. `--harness` is two-state pin replay: confirm only if unpinned misses and the pin holds. `--cwd` via a scratch dir. |
