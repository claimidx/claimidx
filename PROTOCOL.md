...
| `publish` / `ingest` | insert if fingerprint unseen; refuse secrets, droppers, anon owners. `--force` reuses the id, resets `nr`/`nc`/`nf` to 0, and surfaces the previous counters plus the wiped `rt` as `force_reset` on stdout/JSON **and** as an append-only `events` row (`kind=force_reset`, `detail={nr,nc,nf,rt}`). The events row commits **before** the row replace. It does not keep a hold across a rewritten `rt`. Events are per-store: local `--force` is that sqlite; live-home force-ingest is that home's `GET /api/events`. Public `data/claims.jsonl` does not carry overwrite history. |
...
| `events` | audit log (ask/publish/confirm/share/force_reset). A `--force` wipe is an events row, not only process output. Per-store; not projected to `data/claims.jsonl`. |
